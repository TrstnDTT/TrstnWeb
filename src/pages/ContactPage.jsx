...
export default function ContactPage() {
  ...
  return (
    <div className="trstn-ui min-h-[100dvh] bg-[#050506] text-zinc-200 antialiased" style={fontBody}>
      <div
      ...
      <header className="relative z-20 border-b border-white/[0.06] bg-[#050506]/85 backdrop-blur-md">
      ...
      <main className="relative z-10 mx-auto max-w-6xl px-5 pb-24 pt-10 sm:px-8 md:pt-14 lg:pb-32">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-x-16 lg:gap-y-0">
          <div className="lg:col-span-5">
            <motion.h1
              className="text-left text-[clamp(1.85rem,4.2vw,2.65rem)] font-extrabold leading-[1.08] tracking-[-0.04em] text-white"
              style={fontSyne}
            ...
            <ul className="mt-10 space-y-6" role="list">
              <li className="flex items-start gap-3">
                <Mail className="mt-0.5 h-4 w-4 shrink-0 text-zinc-600" strokeWidth={1.5} aria-hidden />
                <div>
                  <p className="text-[10px] uppercase tracking-[0.22em] text-zinc-600">E-mail</p>
                  <ContactLink href={`mailto:${SITE.contactEmail}`}>{SITE.contactEmail}</ContactLink>
                </div>
              </li>
              <li className="flex items-start gap-3">
                <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-zinc-600" strokeWidth={1.5} aria-hidden />
                <div>
                  <p className="text-[10px] uppercase tracking-[0.22em] text-zinc-600">Localisation</p>
                  <p className="text-[14px] text-zinc-400">{locationLabel}</p>
                </div>
              </li>
              <li className="flex items-start gap-3">
                <InstagramGlyph className="mt-0.5 h-4 w-4 shrink-0 text-zinc-600" />
                <div>
                  <p className="text-[10px] uppercase tracking-[0.22em] text-zinc-600">Instagram</p>
              ...
              <AnimatePresence mode="wait">
                {!submitted ? (
                  ...
                  >
                    ...
                    <div className="group relative pt-1">
                      <select
                        id="contact-type"
                        name="projectType"
                        className={`${inputBase} cursor-pointer appearance-none bg-[rgba(255,255,255,0.03)] pr-10 pt-5 text-[14px] ${projectType ? 'text-zinc-100' : 'text-transparent'}`}
                        value={projectType}
                      ...
                      <label
                        ...
                        className={`pointer-events-none absolute left-3 z-[1] text-[13px] transition-all duration-300 ${projectType ? 'top-2 scale-[0.72] text-zinc-400' : 'top-1/2 -translate-y-1/2 text-zinc-500 group-focus-within:top-2 group-focus-within:translate-y-0 group-focus-within:scale-[0.72] group-focus-within:text-zinc-400'}`}
                        style={fontBody}
                      >
                        Type de projet
                      </label>
                    ...
                    <p className="pt-2 text-center text-[12px] leading-relaxed text-zinc-600" style={fontBody}>
                  ...
                  <motion.div
                    ...
                    role="status"
                    aria-live="polite"
                    className="flex min-h-[min(24rem,60vh)] flex-col items-center justify-center rounded-[6px] border-[0.5px] border-white/[0.08] bg-white/[0.03] px-6 py-16 text-center backdrop-blur-sm"
                    initial={prefersReducedMotion ? false : { opacity: 0, y: 24 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.55, ease: easeLux }}
                  >
                    <p
                      className="max-w-md text-[clamp(1.05rem,2.2vw,1.25rem)] font-semibold leading-snug text-zinc-100"
                      style={fontSyne}
                    >
                    ...
                    <p className="mt-4 max-w-sm text-[14px] leading-relaxed text-zinc-500">
                      ...
                      <a
                        ...
                        className="text-zinc-300 underline decoration-white/15 underline-offset-2 transition-colors hover:text-white"
                      >
                        {SITE.contactEmail}
                      </a>
                      .
                    ...
                    <Link
                      to="/"
                      className="trstn-entry-btn mt-10 inline-flex rounded-[6px] border border-white/[0.12] bg-white/[0.05] px-8 py-3 text-[13px] text-zinc-200 backdrop-blur-sm transition-colors hover:bg-white/[0.09] hover:text-white"
                    >
                      Retour à l’accueil
                    </Link>
                  </motion.div>
